const fromCSV = (csv, numFields) => {
  const [header, ...rows] = csv.split("\n");
  const fieldNames = header.split(",").slice(0, numFields);

  return rows.map((row) => {
    const fieldValues = row.split(",").slice(0, numFields);

    return fieldNames.reduce((object, fieldName, index) => {
      return {
        ...object,
