const winnerByYear = (arr, year) => {
  return arr
    .filter((item) => item.year === year) // selecciona solo objetos de ese año
    .sort((a, b) => b.ranking - a.ranking) // ordena por ranking de mayor a menor
    .slice(0, 3) // selecciona los tres primeros (top 3)
    .map((item) => item.name); // extrae solo el nombre
};
